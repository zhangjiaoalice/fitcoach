...
async def create_workout_log(db: AsyncSession, user_id: int, data: CreateWorkoutLog):
    workout_log = await WorkoutLog(user_id=user_id, **data.model_dump())
    db.add(workout_log)
    await db.commit()
    await db.refresh(workout_log)
    return workout_log

async def list_workout_log(db: AsyncSession, user_id: int, log_date: date):
    stmt = select(WorkoutLog).where(User.id == user_id, WorkoutLog.log_date == log_date)
    result = await db.execute(stmt)
    if not result.scalars().all():
        raise ValueError("No workout log found for the given user and date")
